<?php
// Blog Index Page
require_once __DIR__ . '/../../helpers/functions.php';
$pageTitle = 'Blog - Restoran Yönetim Sistemi Rehberleri';
$page = 'blog';
?>

<?php include __DIR__ . '/../layouts/header.php'; ?>

<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        <h1 class="text-4xl font-bold text-gray-800 mb-4">Blog</h1>
        <p class="text-gray-600 text-lg">Restoran yönetimi, dijitalleşme ve teknoloji hakkında güncel içerikler</p>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <!-- Main Content -->
        <div class="lg:col-span-3">
            <?php if (empty($posts)): ?>
                <div class="bg-white rounded-lg shadow p-8 text-center">
                    <p class="text-gray-600">Henüz blog yazısı bulunmamaktadır.</p>
                </div>
            <?php else: ?>
                <div class="space-y-6">
                    <?php foreach ($posts as $post): ?>
                        <article class="bg-white rounded-lg shadow hover:shadow-lg transition-shadow overflow-hidden">
                            <?php if (!empty($post['featured_image'])): ?>
                                <img src="<?php echo htmlspecialchars($post['featured_image']); ?>" 
                                     alt="<?php echo htmlspecialchars($post['title']); ?>" 
                                     class="w-full h-64 object-cover">
                            <?php endif; ?>
                            
                            <div class="p-6">
                                <?php if (!empty($post['category_name'])): ?>
                                    <a href="<?php echo BASE_URL; ?>/blog/category/<?php echo htmlspecialchars($post['category_slug']); ?>" 
                                       class="inline-block bg-blue-100 text-blue-800 text-sm px-3 py-1 rounded-full mb-3 hover:bg-blue-200 transition-colors">
                                        <?php echo htmlspecialchars($post['category_name']); ?>
                                    </a>
                                <?php endif; ?>
                                
                                <h2 class="text-2xl font-bold text-gray-800 mb-3">
                                    <a href="<?php echo BASE_URL; ?>/blog/<?php echo htmlspecialchars($post['slug']); ?>" 
                                       class="hover:text-blue-600 transition-colors">
                                        <?php echo htmlspecialchars($post['title']); ?>
                                    </a>
                                </h2>
                                
                                <?php if (!empty($post['excerpt'])): ?>
                                    <p class="text-gray-600 mb-4"><?php echo htmlspecialchars($post['excerpt']); ?></p>
                                <?php endif; ?>
                                
                                <div class="flex items-center justify-between text-sm text-gray-500">
                                    <div class="flex items-center space-x-4">
                                        <?php if (!empty($post['published_at'])): ?>
                                            <span><?php echo date('d.m.Y', strtotime($post['published_at'])); ?></span>
                                        <?php endif; ?>
                                        <?php if (!empty($post['view_count'])): ?>
                                            <span><?php echo $post['view_count']; ?> görüntüleme</span>
                                        <?php endif; ?>
                                    </div>
                                    <a href="<?php echo BASE_URL; ?>/blog/<?php echo htmlspecialchars($post['slug']); ?>" 
                                       class="text-blue-600 hover:text-blue-800 font-semibold">
                                        Devamını Oku →
                                    </a>
                                </div>
                            </div>
                        </article>
                    <?php endforeach; ?>
                </div>

                <!-- Pagination -->
                <?php if ($totalPosts > 10): ?>
                    <div class="mt-8 flex justify-center">
                        <nav class="flex space-x-2">
                            <?php
                            $totalPages = ceil($totalPosts / 10);
                            $currentPage = $currentPage ?? 1;
                            
                            // Previous
                            if ($currentPage > 1): ?>
                                <a href="<?php echo BASE_URL; ?>/blog?page=<?php echo $currentPage - 1; ?>" 
                                   class="px-4 py-2 bg-white border rounded hover:bg-gray-50">
                                    Önceki
                                </a>
                            <?php endif; ?>
                            
                            <?php for ($i = max(1, $currentPage - 2); $i <= min($totalPages, $currentPage + 2); $i++): ?>
                                <a href="<?php echo BASE_URL; ?>/blog?page=<?php echo $i; ?>" 
                                   class="px-4 py-2 <?php echo $i === $currentPage ? 'bg-blue-600 text-white' : 'bg-white border'; ?> rounded hover:bg-gray-50">
                                    <?php echo $i; ?>
                                </a>
                            <?php endfor; ?>
                            
                            <?php if ($currentPage < $totalPages): ?>
                                <a href="<?php echo BASE_URL; ?>/blog?page=<?php echo $currentPage + 1; ?>" 
                                   class="px-4 py-2 bg-white border rounded hover:bg-gray-50">
                                    Sonraki
                                </a>
                            <?php endif; ?>
                        </nav>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
        </div>

        <!-- Sidebar -->
        <div class="lg:col-span-1">
            <!-- Categories -->
            <?php if (!empty($categories)): ?>
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    <h3 class="text-xl font-bold text-gray-800 mb-4">Kategoriler</h3>
                    <ul class="space-y-2">
                        <?php foreach ($categories as $category): ?>
                            <li>
                                <a href="<?php echo BASE_URL; ?>/blog/category/<?php echo htmlspecialchars($category['slug']); ?>" 
                                   class="text-gray-600 hover:text-blue-600 transition-colors">
                                    <?php echo htmlspecialchars($category['name']); ?>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
            <?php endif; ?>

            <!-- CTA -->
            <div class="bg-blue-600 rounded-lg shadow p-6 text-white">
                <h3 class="text-xl font-bold mb-3">Qordy'yi Deneyin</h3>
                <p class="mb-4">Restoran yönetim sistemimizi ücretsiz deneyin ve dijitalleşmenin farkını yaşayın.</p>
                <a href="<?php echo BASE_URL; ?>/register" 
                   class="inline-block bg-white text-blue-600 px-6 py-2 rounded-lg font-semibold hover:bg-gray-100 transition-colors">
                    Ücretsiz Dene
                </a>
            </div>
        </div>
    </div>
</div>

<?php include __DIR__ . '/../layouts/footer.php'; ?>
