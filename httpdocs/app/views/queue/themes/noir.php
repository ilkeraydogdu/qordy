<?php
/**
 * NOIR theme — yüksek kontrast siyah & gümüş, bar / gece mekânı.
 */
?>
<style>
  body {
  ...
  .aurora { position: fixed; inset: 0; pointer-events: none; filter: blur(88px); opacity: .45; }
  .aurora::before, .aurora::after { content: ''; position: absolute; border-radius: 50%; }
  .aurora::before { width: 50vw; height: 50vw; background: #334155; top: -10vw; left: -10vw; animation: aurora-a 16s ease-in-out infinite alternate; }
  .aurora::after  { width: 45vw; height: 45vw; background: #0ea5e9; bottom: -14vw; right: -6vw; animation: aurora-b 20s ease-in-out infinite alternate; }
  ...
  <?php if ($bgUrl): ?>
  body { background: url('<?php echo qd_safe($bgUrl); ?>') center/cover fixed, #020617; background-blend-mode: overlay; }
  <?php endif; ?>
