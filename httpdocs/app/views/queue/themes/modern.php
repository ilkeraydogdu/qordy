<?php
/**
 * MODERN theme — tech/SaaS feel.
 *
 * Composition: horizontal split (55% QR left, 45% stats right),
 * animated aurora blobs behind glass cards, soft neon accent glow.
 * Inspiration: Linear, Arc, Raycast landing pages.
 */
?>
<style>
  body {
  ...
  /* Animated aurora blobs — purely decorative, GPU-friendly */
  .aurora { position: fixed; inset: 0; pointer-events: none; filter: blur(80px); opacity: .65; }
  .aurora::before, .aurora::after { content: ''; position: absolute; border-radius: 50%; }
  .aurora::before { width: 50vw; height: 50vw; background: var(--accent); top: -10vw; left: -10vw; animation: aurora-a 16s ease-in-out infinite alternate; }
  .aurora::after  { width: 45vw; height: 45vw; background: #3b82f6; bottom: -12vw; right: -8vw; animation: aurora-b 20s ease-in-out infinite alternate; }
  ...
  <?php if ($bgUrl): ?>
  body { background: url('<?php echo qd_safe($bgUrl); ?>') center/cover fixed, #0b1020; background-blend-mode: overlay; }
  <?php endif; ?>
...
<div class="relative box-border h-dvh max-h-dvh min-h-0 w-full grid grid-cols-1 <?php echo $doorShowMetrics ? 'grid-rows-1 max-lg:grid-rows-[1fr_1fr] lg:grid-cols-[1.1fr_0.9fr]' : 'place-content-center max-w-3xl lg:max-w-5xl mx-auto'; ?> gap-2 lg:gap-5 p-2 sm:p-3 lg:p-5 overflow-hidden">
  <!-- Marka + QR: kapı ekranı (müşteri formu değil). Canlı istatistikler kapalıyken sağdaki "operasyon" sütunu yok. -->
  <!-- LEFT: QR hero -->
  ...
  <?php if ($doorShowMetrics): ?>
  <!-- RIGHT: canlı kuyruk / bekleme (isteğe bağlı; açık değilse sadece sol sütun) -->
